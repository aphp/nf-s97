...
== Parser/Serializer
Le projet `afnor-efs` fourni un parseur et un sérialiseur permetant de décoder ou encoder les différents messages définis par les normes NF S97.

Le parseur retourne un objet conforme au modèle défini dans le package `fr.ans.afnor.efs.model`.
...
== Converter
Le projet `afnor-converter` fourni un `Converter` pour être utilisé dans le framework d'intégration `Camel`.
